...
The corpus database is an observational SQLite ledger that tracks all syllable extraction runs across
different extractor tools. It records who ran what extraction, when, with which settings, and what
outputs were produced. This solves the "build provenance drift" problem where you can't remember which
corpus file came from which extraction run.

This is a **build-time observational tool** - it watches and records extraction runs but never
influences extraction behavior. All extractors remain pure, deterministic functions. The ledger just
remembers what happened.

#### The Problem: Build Provenance Drift
...
The ledger answers: *"Run #42 on 2026-01-05 at 14:30 using syllable_extractor v0.2.0 with en_US,
--min 2 --max 8, from /data/corpus/english.txt"*

#### Database Schema

Three simple tables provide complete run tracking:

**`runs`** - One row per extractor invocation:

- Extractor tool name (`syllable_extractor`, `syllable_extractor_nltk`, etc.)
- Version/git SHA for reproducibility
- Hostname (which machine ran this)
...
1. Call `start_run()` at the beginning of extraction
...
The database is small (metadata only, no syllables) and should be committed to version control by
default. It provides valuable build history without bloating the repository.
...
- **Cross-platform** - Paths stored in POSIX format for Windows/Mac/Linux compatibility
- **Queryable** - Easy SQL-like queries for "which run produced this file?"
- **Deterministic** - Same input = same database state
- **Lightweight** - Zero runtime impact on extraction

#### Multi-Tool Support

The `extractor_tool` field distinguishes between different extraction tools. All tools use the same
ledger API and share the same database, enabling cross-tool comparisons:
...
It remembers what you did without telling you how to do it. It provides situational awareness without
imposing workflow. It's a ledger, not a conductor.
